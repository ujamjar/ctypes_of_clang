...
module Make(Clang : Coc_clang.S) = struct
  ...
  let rec ctype ~ctx t =
    ...
    | TArray(t,s) -> 
      if s = 0L then ctype ~ctx (TPtr(t))
              ...
              [%e ctype ~ctx t]]
    | TComp{global} -> evar (name_of_global global)
    | TEnum{global} -> evar (name_of_global global)
  ...
  let gen_enum ~ctx items = 
    ...
    let of_int =
      ...
      Exp.function_ ~loc @@
        (List.map (fun (n,v) -> 
          Exp.case 
            (Pat.constant ~loc (Const.int (Int64.to_int v)))
  ...
  let gen_cstruct ~ctx binding_name name kind members =
    let loc, attrs = ctx.loc, ctx.attrs in
    let pstruct = pvar loc "_ctype" in
    ...
    | exception Not_found -> (*error ~loc "couldn't find '%s'" ci.ci_name*)*)
      gen_cstruct_decl ~ctx binding_name name kind fields_and_obj 
...
*)
  let gen_ccode ~ctx ~code = 
    let gen = function
      ...
      | GFunc { loc; name; typ=TFuncPtr{ret;args;variadic} } -> 
        Some(name, gen_cfn ~ctx name args ret)

      | GComp { name; kind } ->
        let members = [] in (* XXX *)
        Some(name, gen_cstruct ~ctx name name kind members)

      | GEnum { name } ->
        let items = [] in (* XXX *)
        Some(name, gen_enum ~ctx items)

      | GTypedef { name; typ } ->
        Some(name, ctype ~ctx typ)
